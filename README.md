...
| 任务理解 | 逐步推理，每次调用前分析 | 节点逻辑预定义 | **Judge 层全局语义分析，一次判断确定任务类型、复杂度、文档特征** |
...
| 瓶颈风险 | LLM 调用频繁，token 消耗大 | 图结构固化，新任务需重新设计 | **Judge 一次调用，Execute 纯工具执行，LLM 开销最小化** |

**关键差异**：BioHermes 将 LLM 集中用于"判断"和"验证"两端，而非每次工具调用都依赖 LLM 决策。这意味着：
- **更少的 LLM 调用次数** → 更低的延迟和成本
- **更稳定的执行链路** → 工具调用不受 LLM 输出波动影响
- **更可靠的结果质量** → 自动验证而非人工检查
...
2. **LLM 效率最优**：LLM 调用集中在判断和验证两端，执行层纯工具调用。对比 ReAct 每步都需要 LLM 推理，BioHermes 在相同任务下 LLM 调用次数减少 60-80%。
